...
if not hasattr(__builtins__, 'True'):
	# NORMALMENTE REGRESA UN 1 SI TRUE
	# O UN 0 SI FALSE. CREO QUE NO ES
	# REALMENTE NECESARIO
	#True, False = 1, 0
	pass
def isstr(f):
...
class Element:
	...
	def __repr__(self, recursive=0, multiline=0, inprefixes=None):
		...
		def arep(a, inprefixes, addns=1):
			...
			for p in list(self._prefixes.keys()):
				if not p in list(inprefixes.keys()):
					if addns: out += ' xmlns'
	...
	def __call__(self, *_pos, **_set): 
		if _set:
			for k in list(_set.keys()): self._attrs[k] = _set[k]
		if len(_pos) > 1:
...
class Seeder(EntityResolver, DTDHandler, ContentHandler, ErrorHandler):
	...
	def startElementNS(self, name, qname, attrs):
		...
		attrs = dict(attrs)
		newprefixes = {}
		for k in list(self.prefixes.keys()): newprefixes[k] = self.prefixes[k][-1]
...
def unittest():
	...
	try: 
		d._doesnotexist
		raise AttributeError("ExpectedError. ", "but found success. Damn.")
